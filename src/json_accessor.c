...
json_node* get_json_node_from_json_node(json_node* node_p, json_accessor acs, int* non_existing)
{
	...
	while(node_p != NULL && acs.keys_length > 0)
	{
		...
		if(k.is_array_index && node_p->type == JSON_ARRAY)
			node_p = fetch_json_node_from_json_array(node_p, k.index, non_existing);
		else if((!k.is_array_index) && node_p->type == JSON_OBJECT)
			node_p = fetch_json_node_from_json_object(node_p, &(k.key), non_existing);
		else
		{
			(*non_existing) = 1;
			return NULL;
